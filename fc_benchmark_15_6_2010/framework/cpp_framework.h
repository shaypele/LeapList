...
namespace CCP {
	...
	public:
		...
		inline_ static void* char_malloc(const size_t size)	{return malloc(size);}
		inline_ static void  char_free(void* mem)					{free(mem);}

		inline_ static void* char_aligned_malloc(const size_t size) {return ALIGNED_MALLOC(size, CACHELINE_SIZE);}
		inline_ static void* char_aligned_malloc(const size_t size, const size_t alignment) {return ALIGNED_MALLOC(size,alignment);}
		inline_ static void  char_aligned_free(void* mem)	{ALIGNED_FREE(mem);}
	...
	public://general int functionality
		...
		inline_ static int reversechars(const int i) {
			return ((i >> 24)           )  |
				((i >>   8) &   0xFF00)  |
				((i <<   8) & 0xFF0000)  |
				((i << 24));
