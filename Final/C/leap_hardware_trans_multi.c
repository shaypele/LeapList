...
retry_update:
// Decide on the levels of the new nodes and if split is required or not.
	...
	// At this point the code will go back to retry_update to try and update the failed list only.
   status = _xbegin()  ;
	// status is set to _XBEGIN_STARTED when transaction was successful, otherwise it's set to an error code .
    ...
    {
		// If the transcation failed because of _XABORT_CAPACITY MAX_CAPACITY_ABORTS times or more, 
		// it is belived that the transcation wouldn't be able to be complete using hardware transcation. 
		// In this case it should fallback to software transcation.
...
retry_last_remove:
	...
	// If any part of the transcation fails or aborted, all the operations untill that point will be reverted 
	// At this point the code will go back to retry_remove to try and remove keys from the list that the transcation failed at again.
	...
	// Go over all lists and make sure all predeccessors 'next' pointers still point to the excpected nodes. 
		...
		// Check if transcation is still ongoing and end it if it is.	
		if (_xtest())
		{
			_xend();
		}
...
retry_rq:
	...
	// Start a hardware transcation. The goal is to traverse on all nodes without any changes in the way in order to capture a correct snapshot of these set of nodes
	...
	else
	{
		...
		{
			// On failure, restart serach and traverse again.
        	goto retry_rq;
		}
	}
...
/*
 * Add/Update mapping (@k -> @v) into list @l, where  0<= j < size . 
 * @Size - number of elements in the given arrays 
 * j - list index to run update on
 * ptst - per thread state managment, used for memory handling.
 * Update key-value pair on one list only. When this function was changed to support one list only, it affected the hardware transaction somehow and the application didn't work properly.
 * It seems that making the changes and compiling with a lower optimization level fixed the issue.
 * In order to be compatible to all other implementation with the same optimization level, this layout was preserved .
 */
setval_t set_update_SW(set_t *l, setkey_t k, setval_t v, int j,ptst_t   *ptst)
{
	// Preds, succs - two dimensional array, each pred[i]/succ[i] is filled by search_predeccessors
...
// Get the predeccsors (in all of the levels ) of the node that k[j] should be added to into preds[j], get the successors of preds[j] into succs[j].
	...
	// If any part of the transcation fails or aborted, all the operations untill that point will be reverted 
	// At this point the code will go back to retry_update_SW to try again .
    __transaction_atomic 
    { 
	// Go over all lists and make sure all predeccessors 'next' pointers still point to the excpected nodes. 
		// Meaning, point to n[j] until n[j]->level and point to succs[j][i] at higher levels.
...
/*
 * Remove mapping for key @k from set @l, where  0<= j < size.
 * @Size - number of elements in the given arrays 
* j - list index to run update on
 * ptst - per thread state managment, used for memory handling.
 * Update key-value pair on one list only. When this function was changed to support one list only, it affected the hardware transaction somehow and the application didn't work properly.
 * It seems that making the changes and compiling with a lower optimization level fixed the issue.
 * In order to be compatible to all other implementation with the same optimization level, this layout was preserved .
 * Returns -1 if key not found, 0 for a successful remove.
 */
setval_t set_remove_SW(set_t *l, setkey_t k, int j,ptst_t *ptst)
{
	// Preds, succs - two dimensional array, each pred[i]/succ[i] is filled by search_predeccessors
...
retry_last_remove_SW:
	...
	// If any part of the transcation fails or aborted, all the operations untill that point will be reverted 
	// At this point the code will go back to retry_remove to try  again.
    __transaction_atomic 
    {
		// Go over all lists and make sure all predeccessors 'next' pointers still point to the excpected nodes. 
		// If old_node[j][0] or old_node[j][1] ( if there's a merge ) or any of the predeccessors or successors are not live any more, abort the transcation.
