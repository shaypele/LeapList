...
public class LeapListDB {
			...
			newKeys[i] = keys[i] + 2 ; // avoid sentinel; 
				...
				do
				{
					...
					{//if updateLT throw TransactionException it means it didn't run atomically and rolled back
						//so we restart the loop and run only on the current LeapList again until we succeed.
	 ...
	 * we try this function only once and if it not succeed we start trying run again only for the current LeapList,
	    ...
	    for(int j=0; j<size; j++)
        {
		    ...
		    newKeys[j] = keys[j]+2; // Avoid sentinel
		    	...
		    	do{
		    		n = new LeapNode();
			    	stopLoop=true;
				    RemoveSetup(ll[j],newKeys[j], size, pa, na, n, oldNode, merge, changed,j);
				    try
				    {//if RemoveLT throw TransactionException it means it didn't run atomically and rolled back
						//so we restart the loop and run over only the current LeapList again until we succeed.
	 ...
	 */
	private void RemoveSetup(LeapList l, long key,int size, LeapNode[] pa,
	 ...
	 * all over again for all the LeapLists.
	 * we try this function only once and if it not succeed we start trying all over again only for the current LeapList,
	 * after check several numbers of retries 1 gives the best performance because in the most cases we need to use searchPredecessor again
	 * because the list is changed.
	 */
	@Atomic(retries = 1)
