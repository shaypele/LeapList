...
restart_look:

    {
        ...
        for ( i = MAX_LEVEL - 1; i >= 0; i-- )
        {
			...
			// If pointer to next node is marked then restart search.
...
void set_update(set_t **l, setkey_t *k, setval_t *v, int size)
{
    ...
    vwLock my_order;
	//Acquire global lock. This lock blocks Update & Remove & Range Query & Lookup operations from other threads.
	...
	// Release global lock - allow any thread to obtain the lock.
...
void set_remove(set_t **l, setkey_t *k, int size)
{
    ...
    for(j=0; j<size; j++)
    {
        ...
        /* If the key is not present, just continue */
