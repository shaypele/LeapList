...
static void OrderedLock_Acquire(vwLock *my_order,volatile vwLock lock,volatile vwLock* lockSyncVars)
{
        int idx;

//	printf("bef lock\n");

	*my_order = fetch_and_add(&lock, 1);
	idx = *my_order % MAX_ALL_THREADS;//g_num_of_threads;
	
	while (lockSyncVars[idx] != *my_order);
//	printf("afterrrr lock \n");
}

static void OrderedLock_Release(vwLock *my_order,volatile vwLock* lockSyncVars)
{
	int next = *my_order + 1;
	int idx = next % MAX_ALL_THREADS; //g_num_of_threads;
	lockSyncVars[idx] = next; 
}
...
{
	OrderedLock_Acquire(my_order,order,sync_vars);
}
static void OrderedRWLock_ReleaseWrite(vwLock *my_order)
{
	OrderedLock_Release(my_order,sync_vars);
}
